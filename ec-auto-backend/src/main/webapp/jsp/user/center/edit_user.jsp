...
<div id="edit_dlg" class="easyui-dialog" title="编辑用户" style="padding:10px;width:300px;"
	    ...
	    <form id="edit_form" method="post" style="display:block;">
	    	<table>
	    		...
	    		<tr>
	    			<td>密码:</td>
	    			<td><input class="easyui-validatebox" id="edit_passwd" type="password" name="passwd" data-options="required:true"></input></td>
	    		</tr>
	    		<tr>
	    			<td>重复密码:</td>
	    		...
	    		<tr>
	    			<td>邮箱:</td>
	    			<td><input class="easyui-validatebox" name="email" data-options="required:true,validType:'email'"></input></td>
	    		</tr>
	    		<tr>
	    			<td>厂牌:</td>
	    			<td>
	    				<select class="search-field"   name="brandId" style="width:150px;" onchange="selectSerialIdByBrandIdEdit();">
	    		...
	    		<tr>
	    			<td>车系:</td>
	    			<td>
	    				<select class="search-field"  id="edit_seria_id" name="serialId" style="width:150px;">
	    					<c:forEach var="seriallists" items="${carSerialLists}">
		    					<option value="${seriallists.id}">${seriallists.name}</option>
		    				</c:forEach>	
	    				</select>
	    			</td>
	    		</tr>
...
<script type="text/javascript">
		...
		function doEditUser () {			
			 ...
			 AjaxUtils.ajaxFormSubmit($form, "/user/doedit", function(json) {
					if (json.succ=='Y') {
		...
		function selectSerialIdByBrandIdEdit() {
