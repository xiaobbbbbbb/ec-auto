...
public class LoginStatusCheckFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse resp,
        ...
        if(requestURI.contains("/raluser/dologin") || requestURI.contains("/raluser/logout") || isStaticResources(requestURI)){
            chain.doFilter(request, response);
            return;
        }
        
	    RalUser user = (RalUser) request.getSession().getAttribute(WebUtils.SESSION_KEY_USER);
	    if (user==null) {
