...
import com.ecarinfo.auto.front.util.WebUtils;
...
public class LoginStatusCheckFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse resp,
        ...
        if(requestURI.contains("/index/login") || requestURI.contains("/index/dologin") || isStaticResources(requestURI)){
            chain.doFilter(request, response);
            return;
        }
        
	    UserInfo user = (UserInfo) request.getSession().getAttribute(WebUtils.SESSION_KEY_USER);
	    if (user==null) {
