	...
	<center>
		...
		<div class="all">
			<div class="detail_center">
				<span class="detail_center_tag"><a href="${ctx }/index/">首页</a> 
				<c:if test="${type ==1 }">
				    &gt;
			        <a href="${ctx }/discuss/index">产品评价</a>  
			        &gt; 
			        <a href="${ctx }/discuss/list">车友口碑明细</a>
			    </c:if>
				<c:if test="${type ==0 }">
				    &gt;
			       <a href="${ctx }/downside/index">负面情报</a>  
			        &gt; 
...
<script type="text/javascript">
	...
	function loadArticleById() {
		...
		var url = '${ctx}/discuss/article?aid='+a_id+'&cpage='+getArticlePage();
		$.getJSON(url, function(data) {
			var sp ="&nbsp;&nbsp;";
			$('.a_title').text(data.title);
			$('.a_mediaName').html(sp+(data.mediaName?data.mediaName:"暂无")+sp+sp+sp);
			$('.a_ptime').html(data.showArticleCtime+sp+sp+sp);
			$('.a_content').html(sp+sp+sp+data.simpleContent);
