...
function add_update(url,tit){
	art.dialog.open(url, {
	    ...
	    ok: function () {
	    	...
	    	var reqUrl=""
	    	/* $.post("${ctx}/group/update",param,function(json){
				 if(json.success){
					window.location.href="${ctx}/backend/org_list";
				 }else{
     		...
     		<td class="b_right" valign="top"  >
     			...
     			<div class="b_c_second_title">
     				...
     				<div class="second_title_r">
     					...
     					<!-- <a href="javascript:void(0);" id="search_btn" >检索</a> -->
     				</div>
     				<div class="clear" ></div>
     			</div>
			              ...
			              <tbody>
			              	<c:forEach items="${groups}" var="dto" varStatus="sn" >
				              	<tr>
				                   ...
				                   <td>${dto.name }</td>
				                   <td>${dto.name }</td>
				                   <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${dto.createTime }" /></td>
				                   <td><a href="javascript:void(0);" onclick="update(${dto.groupId })">编辑</a></td>  
				                </tr>
			              	</c:forEach>
			              </tbody>
     				</table>
