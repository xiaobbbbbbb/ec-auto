	...
	<div data-role="page" id="search_page">
		...
		<div data-role="content">
			...
			<select id="search_level" name="searchLevel">		
				...
				<option value="1" <#if searchLevel=1> selected="selected" </#if>>一般</option>		
				<option value="2" <#if searchLevel=2> selected="selected" </#if>>中等 </option>		
				<option value="3" <#if searchLevel=3> selected="selected" </#if>>严重  </option>						
			</select> 
			日期
		...
		<script type="text/javascript">
			...
			function searchRecord() {
				...
				var url = '/article/bad_article_index?page=1&userId=${userId?c}&brandId=' + parseInt($('#brandId').val());
